...
/*   Updated: 2024/10/28 14:55:30 by aerrfig          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_minimap(t_cub3d *prog)
{
	...
	while (i_map < MINIPAM_H)
	{
		...
		{
			if (j < 0 || i < 0 || j >= GRID_SIZE*(prog->map_size.x) || i >= GRID_SIZE*(prog->map_size.y))
			...
			else
			{
				...
				else if (prog->map[(int)(i/GRID_SIZE)][(int)(j/GRID_SIZE)] == ' ')
					my_mlx_pixel_put(&prog->img_data, j_map, i_map, 0x0000FF);
				else
	...
	draw_rectangle(&prog->img_data, (t_vec){(M_MAP_HALF), (M_MAP_HALF)}, 0x00FF00, 10);
}
